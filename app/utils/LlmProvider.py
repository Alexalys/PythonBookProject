...
from app.configuration.exceptions import LlmException
...
class LlmProvider:
    ...
    def generate_book_info(self, book: BookCreateSchema) -> BookModel:
        """
        Generate detailed book information using OpenAI GPT-3.5.

        Args:
            book (BookCreateSchema): A schema representing the book information.

        Returns:
            BookModel: A BookModel object populated with information generated by OpenAI.

        Raises:
            LlmException: If multiple books are found in the response.
        """
        ...
        if isinstance(content, list) and len(content) > 1:
            logger.error("Error during LLM request")
            error: LlmException = LlmException()
    ...
    def generate_reccomendations(self, books: list[BookModel]) -> list[BookModel]:
        """
        Generate book recommendations based on previously read books using OpenAI GPT-3.5.

        Args:
            books (list[BookModel]): A list of BookModel objects representing books already read.

        Returns:
            list[BookModel]: A list of BookModel objects representing recommended books.
